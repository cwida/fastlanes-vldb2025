...
def generate_latex_table(df, caption="Aggregated File Sizes", label="tab:table_1"):
    ...
    df.index = df.index.str[:5]  # Keep only first 5 characters of table names
...
def load_csv(file_path):
    ...
    if file.exists():
        ...
        expected_columns = {"table_name", "version", "decompression_time_ms"}
        ...
        print(f"⚠️ File not found: {file_path}")
        return pd.DataFrame(columns=["table_name", "version", "decompression_time_ms"])
...
def load_all_datasets(script_dir: Path) -> dict:
    ...
    fastlanes_result_path = script_dir / "../fastlanes/benchmark/result/simd_benchmark/public_bi/fastlanes_20_avx512dq.csv"
    btrblocks_path = script_dir / "../btrblocks/result/avx512dq/public_bi/btrblocks.csv"
...
def combine_check_and_divide_by_repetition(dataframes):
    ...
    2. If a row has 'n_repetition' and 'decompression_time_ms', we divide
       that row's 'decompression_time_ms' by row['n_repetition'] (if non-zero).
    3. Similarly for 'n_repetition_full_scan' → 'decompression_time_ms',
    ...
    if "n_repetition" in combined_df.columns and "decompression_time_ms" in combined_df.columns:
        def _divide_decomp(row):
            if pd.notna(row.get("n_repetition", None)) and row["n_repetition"] != 0:
                return row["decompression_time_ms"] / row["n_repetition"]
            return row["decompression_time_ms"]

        combined_df["decompression_time_ms"] = combined_df.apply(_divide_decomp, axis=1)
        print("✅ Divided 'decompression_time_ms' by each row's 'n_repetition' if non-zero.")

    # For a separate full-scan repetition count
    if "n_repetition_full_scan" in combined_df.columns and "decompression_time_ms" in combined_df.columns:
        def _divide_full_scan(row):
            if pd.notna(row.get("n_repetition_full_scan", None)) and row["n_repetition_full_scan"] != 0:
                return row["decompression_time_ms"] / row["n_repetition_full_scan"]
            return row["decompression_time_ms"]

        combined_df["decompression_time_ms"] = combined_df.apply(_divide_full_scan, axis=1)
        print("✅ Divided 'decompression_time_ms' by each row's 'n_repetition_full_scan' if non-zero.")
...
def aggregate_decompression_time_mss_1(dataframes):
    """
    1. Combine and (potentially) divide by repetition columns row-by-row.
    2. Pivot on 'decompression_time_ms'.
    ...
    # Pivot only on 'decompression_time_ms' as the values
    pivot_df = combined_df.pivot_table(
        index="table_name",
        columns="version",
        values="decompression_time_ms",
        aggfunc="sum",
        fill_value=0
    )
...
def normalize_decompression_time_mss(df):
    """
...
def main_table_1(datasets: dict, script_dir: Path):
    ...
    aggregated_df = aggregate_decompression_time_mss_1([
        dwdd_parquet_df,
    ...
    if not aggregated_df.empty:
        ...
        normalized_df = normalize_decompression_time_mss(aggregated_df.copy())
...
def aggregate_decompression_time_mss_2(dataframes):
    """
...
def compute_decompression_times(df):
    """
    ...
    if "uncompressed" in df.columns:
        ...
        print("⚠️ Column 'uncompressed' not found. Skipping decompression time ratio computation.")
    return df
...
def main_table_2(datasets: dict, script_dir: Path):
    ...
    aggregated_df = aggregate_decompression_time_mss_2([
        dwdd_df,
        dwdd_forced_df,
        fastlanes_df,
        dwdd_pyarrow_df,
    ...
    if not aggregated_df.empty:
        ...
        # Example usage of compute_decompression_times:
        decompression_time_df = compute_decompression_times(aggregated_df.copy())
        print_markdown_table_2(decompression_time_df)
